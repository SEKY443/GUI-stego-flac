...
By default the carrier is tagged as a generic "stego-flac carrier" — a plain
Vorbis TITLE/DESCRIPTION rather than anything from the cover. `--cover
--keep-cover-metadata` copies the cover's own tags (title, artist, album, ...)
into the output instead, so a player or library scanner sees the cover's real
metadata rather than a giveaway placeholder:

```sh
stego-flac encode secret.bin --cover voice.wav --keep-cover-metadata
```

`AUDIOMODEM_PLAN` and `AUDIOMODEM_PROFILE` — the tags `decode` and `info` rely
on to configure themselves — are never overwritten by the cover's tags, so
this cannot break decoding regardless of what the cover happens to be tagged
with.
