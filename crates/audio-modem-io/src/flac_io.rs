...
//!
//! This lives outside `audio-modem-core` on purpose: the core crate is
//! container-independent, and keeping codec dependencies out of it means the
//! modem can be reused over WAV, over a raw socket, or over a sound card
//! without dragging FLAC along.
//!
...
pub struct FlacAudio {
    ...
    pub sample_rate: u32,
    pub channels: usize,
    /// Non-fatal notes gathered while reading, e.g. a truncated tail that FEC
    /// may still recover from.
    ///
    /// A CLI can print these to stderr the same way the original inline
    /// `eprintln!` calls did; a GUI has no stderr to print into, so they are
    /// surfaced here instead of as a side effect.
...
pub fn read_flac(path: &Path) -> Result<FlacAudio> {
    ...
    loop {
        let packet = match reader.next_packet() {
            ...
            Err(error) if !samples.is_empty() => {
                warnings.push(format!(
                    "{} ends mid-frame ({error}); decoding what survived",
                    path.display()
                ));
                break;
            }
        ...
        let decoded = match decoder.decode(&packet) {
            ...
            Err(error) if !samples.is_empty() => {
                warnings.push(format!(
                    "{} has a damaged frame ({error}); decoding what survived",
                    path.display()
                ));
                break;
            }
