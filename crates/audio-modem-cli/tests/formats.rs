...
/// [`voice_wav`], with a RIFF `LIST`/`INFO` chunk carrying a title and artist,
/// to exercise `--keep-cover-metadata`.
fn voice_wav_with_tags(seconds: usize, title: &str, artist: &str) -> Vec<u8> {
    ...
    let mut info = info_subchunk(b"INAM", title);
    info.extend(info_subchunk(b"IART", artist));
    ...
    // Splice the LIST chunk in right after the RIFF/WAVE header (12 bytes:
    // "RIFF" + size + "WAVE"), then patch the RIFF size to account for it.
    let plain = voice_wav(seconds);
    let mut wav = plain[..12].to_vec();
    wav.extend_from_slice(&list);
    wav.extend_from_slice(&plain[12..]);
...
#[test]
fn keep_cover_metadata_copies_the_covers_own_tags() {
    let dir = TempDir::new("cover-keep-tags");
    let (input, cover, carrier) = (dir.join("m.bin"), dir.join("v.wav"), dir.join("c.flac"));
    fs::write(&input, fixtures::incompressible(40_000, 5)).unwrap();
    fs::write(
        &cover,
        voice_wav_with_tags(3, "Midnight Radio Test", "The Coverband"),
    )
    .unwrap();
    ...
    let out = encode(
        &input,
        &carrier,
        &["--cover", cover.to_str().unwrap(), "--keep-cover-metadata"],
    );
    assert!(out.status.success(), "encode failed: {}", stderr(&out));
    ...
    assert!(
        text.contains("TITLE=Midnight Radio Test"),
        "the cover's title should replace the placeholder, got:\n{text}"
    );
    assert!(
        text.contains("ARTIST=The Coverband"),
        "the cover's artist should be copied over, got:\n{text}"
    );
    // The tone plan is what makes the carrier decodable at all, so a cover's
    // own tags must never be able to overwrite it.
    assert!(
        text.contains("AUDIOMODEM_PLAN=mode=ofdm"),
        "the load-bearing plan tag must survive, got:\n{text}"
    );

    // And the round trip must still work: inheriting the cover's tags must
    // not disturb the plan those tags could have collided with.
    let landing = dir.join("out.bin");
    let out = decode(&carrier, &landing, &[]);
    assert!(out.status.success(), "decode failed: {}", stderr(&out));
    assert_eq!(
        fs::read(&landing).unwrap(),
        fixtures::incompressible(40_000, 5)
    );
}

#[test]
fn without_the_flag_the_covers_tags_are_left_out() {
    let dir = TempDir::new("cover-no-keep-tags");
    let (input, cover, carrier) = (dir.join("m.bin"), dir.join("v.wav"), dir.join("c.flac"));
    fs::write(&input, fixtures::incompressible(4_000, 6)).unwrap();
    fs::write(
        &cover,
        voice_wav_with_tags(3, "Should Not Appear", "Nor This"),
    )
    .unwrap();
    ...
    let text = String::from_utf8_lossy(&raw[..raw.len().min(4096)]);
    assert!(
        text.contains("TITLE=stego-flac carrier"),
        "without the flag the placeholder title should stay, got:\n{text}"
    );
    assert!(
        !text.contains("Should Not Appear") && !text.contains("Nor This"),
        "the cover's tags must not leak in without --keep-cover-metadata, got:\n{text}"
    );
}

#[test]
fn keep_cover_metadata_requires_a_cover() {
    let dir = TempDir::new("cover-keep-tags-noflag");
    let input = dir.join("m.bin");
    fs::write(&input, b"hello").unwrap();

    let out = encode(&input, &dir.join("c.flac"), &["--keep-cover-metadata"]);
    assert!(
        !out.status.success(),
        "--keep-cover-metadata without --cover should be refused"
    );
}
