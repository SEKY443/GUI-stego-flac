...
pub struct EncodeArgs {
    ...
    /// Copy the cover audio's own tags (title, artist, album, ...) into the
    /// carrier, in place of the default "stego-flac carrier" placeholder.
    ///
    /// A carrier that inherits the cover's real metadata is a better disguise
    /// than one that announces itself in its own tags. `AUDIOMODEM_PLAN` and
    /// `AUDIOMODEM_PROFILE` are never overwritten, whatever the cover's own
    /// tags happen to contain.
...
impl PlanArgs {
    ...
    pub fn is_explicit(&self) -> bool {
        self.to_overrides().is_explicit()
    }

    fn to_overrides(&self) -> audio_modem_io::PlanOverrides {
        audio_modem_io::PlanOverrides {
            profile: self.profile.map(Profile::from),
    ...
    /// override what the file says. The resolution order and cross-field
    /// validation live in [`audio_modem_io::PlanOverrides`], shared with the
    /// desktop GUI so both front ends behave identically.
